...
On confirm, run the **task-parallel** fan-out for the ticked candidates — you've already produced the task list, so hand it straight to that flow (which starts by loading the **`cmux` skill** for the command reference + focus rules — do that before opening any surface; if that skill isn't installed, fall back to the `cmux …` commands documented there with `cmux --help` as the authority). Do task-parallel's **Step 2 first** (resolve the shared setup ONCE and pass it down so no runner ever prompts): the **required** tracker mapping (`TRACKER`/`TEAM`/`PROJECT`), the base branch, the per-task web/sim target, the qa-port/creds, and the per-target resource key. Then per pick:
