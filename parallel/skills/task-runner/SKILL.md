...
**Detect cmux by `CMUX_WORKSPACE_ID`** (set in every cmux surface — the reliable signal; don't probe `/tmp/cmux.sock`). Set your status through every phase so the parent and user can watch the sidebar:
```bash
