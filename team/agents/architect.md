...
- **grill-me check — MANDATORY, before anything else in this phase.** Look for a **grill-me** skill/plugin (in your own skill list, or ask the parent whether one exists in the session). If grill-me is available, you MUST NOT run the questioning yourself: stop, return to the parent with your gathered context and remaining open points, and explicitly instruct the parent to invoke the **grill-me** skill (Skill tool) to interrogate the user, then send the answers back to you (SendMessage) before you proceed to Phase 2. Never substitute your own ad-hoc question list when grill-me exists.
- Only if no grill-me skill exists anywhere in the session, build the question list yourself. Cover whichever apply: exact user flows and actors; priority/ordering when the task has parts; what "done" looks like (observable acceptance); design intent where Figma is ambiguous or absent; every edge case — empty/first-run state, concurrency/races, partial failure, offline, permissions/roles, timezone, i18n, money precision, pagination limits; non-goals; migration/backfill and rollout; performance budgets.
- You cannot talk to the user directly — the parent is always the mouthpiece. Return open questions as a numbered list with your best-guess default per question, and instruct the parent to put them to the user (via grill-me when available, AskUserQuestion otherwise) and send the answers back to you (SendMessage) before you finalize. Material ambiguities (ones that change the design) are never resolved by silent assumption — minor ones may proceed on a stated default.
