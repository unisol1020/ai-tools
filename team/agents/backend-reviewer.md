...
Read-only inspection only: `git diff`/`git log`/`git status`, `rg`, `ast-grep`, `codegraph explore`, `graphify query`, `mempalace search`, `find`, `ls`, `wc`. Prefer the `Read` tool for files you'll cite. **Never** run anything that mutates state — no installs, no build/test/migration commands, no dev servers, no writes to any database.
